---
description: Create or switch to chore/ worktree
---
...
2. **If description IS provided:**
   ...
   a. **Sanitize description**
      - Input: $ARGUMENTS (e.g., "Update Dependencies")
      - Steps:
        1. Convert to lowercase
        2. Replace spaces with hyphens
        3. Remove special characters except hyphens
        4. Remove leading/trailing hyphens
      - Example: "Update Dependencies" → "update-dependencies"
...
3. **If description is NOT provided (interactive mode):**
   ...
   a. **List existing chore/ worktrees**
      - Run: `git worktree list --porcelain`
      - Parse to find worktrees with branches starting with "chore/"
      - Store as CHORE_WORKTREES

   b. **Present options to user**
      - If CHORE_WORKTREES is not empty:
        - Format as numbered list:
          ```
          📋 Worktrees chore/ existentes:
          1. chore/update-dependencies → /path/to/chore/update-dependencies
          2. chore/cleanup-code → /path/to/chore/cleanup-code
          [{count+1}]. Criar novo worktree chore/
          ```
      ...
      - If CHORE_WORKTREES is empty:
        - Output: "ℹ️ Nenhum worktree chore/ encontrado"
        - GO TO step 3d
   ...
   d. **Ask for new description**
      - Use AskUserQuestion:
        ```
        Digite a descrição da tarefa de manutenção:
        (Será convertido para: chore/[sua-descricao])
        ```
      - Get user input
...
1. **Success message**
   - Output: "✅ Worktree chore/ pronto"
   - Output: "📍 Branch: {branch_name}"
   - Output: "📂 Path: {worktree_path}"

## Key Rules

- **PREFIX**: Always use "chore/" prefix for maintenance task branches
- **SANITIZATION**: Convert to lowercase, spaces→hyphens, remove special chars
- **OPTIONAL DESCRIPTION**: Support both `/gitworktree:chore [description]` and `/gitworktree:chore` (interactive)
- **REUSE**: Call unified `/gitworktree` command, don't duplicate logic
- **LISTING**: Only show worktrees with "chore/" prefix when interactive
