---
description: Create or switch to docs/ worktree
---
...
2. **If description IS provided:**
   ...
   a. **Sanitize description**
      - Input: $ARGUMENTS (e.g., "API Documentation Update")
      - Steps:
        1. Convert to lowercase
        2. Replace spaces with hyphens
        3. Remove special characters except hyphens
        4. Remove leading/trailing hyphens
      - Example: "API Documentation Update" → "api-documentation-update"
...
3. **If description is NOT provided (interactive mode):**
   ...
   a. **List existing docs/ worktrees**
      - Run: `git worktree list --porcelain`
      - Parse to find worktrees with branches starting with "docs/"
      - Store as DOCS_WORKTREES

   b. **Present options to user**
      - If DOCS_WORKTREES is not empty:
        - Format as numbered list:
          ```
          📋 Worktrees docs/ existentes:
          1. docs/api-documentation → /path/to/docs/api-documentation
          2. docs/readme-update → /path/to/docs/readme-update
          [{count+1}]. Criar novo worktree docs/
          ```
      ...
      - If DOCS_WORKTREES is empty:
        - Output: "ℹ️ Nenhum worktree docs/ encontrado"
        - GO TO step 3d
   ...
   d. **Ask for new description**
      - Use AskUserQuestion:
        ```
        Digite a descrição da documentação:
        (Será convertido para: docs/[sua-descricao])
        ```
      - Get user input
...
1. **Success message**
   - Output: "✅ Worktree docs/ pronto"
   - Output: "📍 Branch: {branch_name}"
   - Output: "📂 Path: {worktree_path}"

## Key Rules

- **PREFIX**: Always use "docs/" prefix for documentation branches
- **SANITIZATION**: Convert to lowercase, spaces→hyphens, remove special chars
- **OPTIONAL DESCRIPTION**: Support both `/gitworktree:docs [description]` and `/gitworktree:docs` (interactive)
- **REUSE**: Call unified `/gitworktree` command, don't duplicate logic
- **LISTING**: Only show worktrees with "docs/" prefix when interactive
