...
import { toCreateItemRow, toDomainBookItem, toUpdateItemRow } from '@data/mappers';

import type { DrizzleDB } from '../drizzle';
import { items } from '../drizzle';

export class DrizzleBookItemRepository implements BookItemRepository {
  constructor(private db: DrizzleDB) {}

  async create(input: CreateBookItemInput): Promise<BookItem> {
    const id = uuid();
    ...
    if (!created) {
      throw new Error('Failed to create book item');
    }
    return created;
  }

  async getById(id: string): Promise<BookItem | null> {
    const result = await this.db
      .select()
      .from(items)
      .where(and(eq(items.id, id), eq(items.type, 'book')))
      .limit(1);
    if (result.length === 0) {
      return null;
    }
    return this.mapRowToBookItem(result[0]);
  }

  async getAll(
    sort?: SortCriteria<ItemSortField>,
    pagination?: PaginationParams,
  ): Promise<PaginatedResult<BookItem>> {
    ...
    const [rows, totalResult] = await Promise.all([
      this.db
        .select()
        .from(items)
        .where(eq(items.type, 'book'))
        .orderBy(orderDirection(orderByColumn))
        .limit(limit)
        .offset(offset),
      this.db.select({ count: count() }).from(items).where(eq(items.type, 'book')),
    ]);

    const totalCount = totalResult[0]?.count ?? 0;

    return {
      items: rows.map((row) => this.mapRowToBookItem(row)),
      totalCount,
      pagination: {
        offset,
        limit,
  ...
  async update(id: string, updates: UpdateBookItemInput): Promise<BookItem | null> {
  ...
  async delete(id: string): Promise<boolean> {
    const result = await this.db.delete(items).where(and(eq(items.id, id), eq(items.type, 'book')));
    return result.changes > 0;
  }

  async getByListId(listId: string): Promise<BookItem[]> {
    const rows = await this.db
      .select()
      .from(items)
      .where(and(eq(items.listId, listId), eq(items.type, 'book')))
      .orderBy(asc(items.sortOrder));
    return rows.map((row) => this.mapRowToBookItem(row));
  }

  async getBySectionId(sectionId: string): Promise<BookItem[]> {
    const rows = await this.db
      .select()
      .from(items)
      .where(and(eq(items.sectionId, sectionId), eq(items.type, 'book')))
      .orderBy(asc(items.sortOrder));
    return rows.map((row) => this.mapRowToBookItem(row));
  }

  async search(
    criteria: BookItemFilterCriteria,
    sort?: SortCriteria<ItemSortField>,
    pagination?: PaginationParams,
  ): Promise<PaginatedResult<BookItem>> {
    const offset = pagination?.offset ?? 0;
    ...
    const conditions = [eq(items.type, 'book')];
    if (criteria.query) {
  ...
  async groupBy(criteria: ItemGroupCriteria): Promise<GroupResult<BookItem, ItemGroupCriteria>> {
    const allItems = await this.db.select().from(items).where(eq(items.type, 'book'));

    const grouped: GroupResult<BookItem, ItemGroupCriteria> = {};
    ...
    for (const row of allItems) {
      ...
      switch (criteria) {
        ...
        case 'sectionId':
          ...
          break;
        case 'isChecked':
          key = item.isChecked ? 'read' : 'unread';
          break;
        case 'createdAt':
  ...
  private mapRowToBookItem(row: typeof items.$inferSelect): BookItem {
    return toDomainBookItem({
      id: row.id,
      list_id: row.listId,
