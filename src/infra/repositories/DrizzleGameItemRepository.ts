...
import { toCreateItemRow, toDomainGameItem, toUpdateItemRow } from '@data/mappers';

import type { DrizzleDB } from '../drizzle';
import { items } from '../drizzle';

export class DrizzleGameItemRepository implements GameItemRepository {
  constructor(private db: DrizzleDB) {}

  async create(input: CreateGameItemInput): Promise<GameItem> {
    const id = uuid();
  ...
  async getById(id: string): Promise<GameItem | null> {
    const result = await this.db
      .select()
      .from(items)
      .where(and(eq(items.id, id), eq(items.type, 'game')))
      .limit(1);
    if (result.length === 0) {
      return null;
    }
    return this.mapRowToGameItem(result[0]);
  }

  async getAll(
    sort?: SortCriteria<ItemSortField>,
    pagination?: PaginationParams,
  ): Promise<PaginatedResult<GameItem>> {
    ...
    const [rows, totalResult] = await Promise.all([
      this.db
        .select()
        .from(items)
        .where(eq(items.type, 'game'))
        .orderBy(orderDirection(orderByColumn))
        .limit(limit)
        .offset(offset),
      this.db.select({ count: count() }).from(items).where(eq(items.type, 'game')),
    ]);

    const totalCount = totalResult[0]?.count ?? 0;

    return {
      items: rows.map((row) => this.mapRowToGameItem(row)),
      totalCount,
      pagination: {
        offset,
        limit,
  ...
  async update(id: string, updates: UpdateGameItemInput): Promise<GameItem | null> {
  ...
  async delete(id: string): Promise<boolean> {
    const result = await this.db.delete(items).where(and(eq(items.id, id), eq(items.type, 'game')));
    return result.changes > 0;
  }

  async getByListId(listId: string): Promise<GameItem[]> {
    const rows = await this.db
      .select()
      .from(items)
      .where(and(eq(items.listId, listId), eq(items.type, 'game')))
      .orderBy(asc(items.sortOrder));
    return rows.map((row) => this.mapRowToGameItem(row));
  }

  async getBySectionId(sectionId: string): Promise<GameItem[]> {
    const rows = await this.db
      .select()
      .from(items)
      .where(and(eq(items.sectionId, sectionId), eq(items.type, 'game')))
      .orderBy(asc(items.sortOrder));
    return rows.map((row) => this.mapRowToGameItem(row));
  }

  async search(
    criteria: GameItemFilterCriteria,
    sort?: SortCriteria<ItemSortField>,
    pagination?: PaginationParams,
  ): Promise<PaginatedResult<GameItem>> {
    const offset = pagination?.offset ?? 0;
    ...
    const conditions = [eq(items.type, 'game')];
    if (criteria.query) {
  ...
  async groupBy(criteria: ItemGroupCriteria): Promise<GroupResult<GameItem, ItemGroupCriteria>> {
    const allItems = await this.db.select().from(items).where(eq(items.type, 'game'));

    const grouped: GroupResult<GameItem, ItemGroupCriteria> = {};

    for (const row of allItems) {
      const item = this.mapRowToGameItem(row);
      let key: string;
      ...
      switch (criteria) {
        ...
        case 'sectionId':
          ...
          break;
        case 'isChecked':
          key = item.isChecked ? 'played' : 'unplayed';
          break;
        case 'createdAt':
  ...
  private mapRowToGameItem(row: typeof items.$inferSelect): GameItem {
    return toDomainGameItem({
      id: row.id,
      list_id: row.listId,
